-- Migration 001: Schema Enhancements for CTCAE v6.0 Support (FIXED)
-- Run date: 2025-12-13
...
-- Note: Removed IFNULL() as it's not supported in index definitions in all MySQL versions
ALTER TABLE dictionary_event
  DROP INDEX uniq_event;

-- Create the new unique index without IFNULL
-- MySQL treats NULL as distinct, so multiple NULL values are allowed
ALTER TABLE dictionary_event
  ADD UNIQUE KEY uniq_event_versioned (diagnosis, icd10, source, ctcae_version);
